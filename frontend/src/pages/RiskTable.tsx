...
export const RiskTable: React.FC<RiskTableProps> = ({
  ...
  const handleFieldBlur = async (riskId: string, field: keyof Risk, value: string) => {
    try {
      ...
      const response = await fetch(`/api/risks/${riskIndex}/update`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
                  ...
                  <tr key={risk.id} className={risk.isSelected ? "selected" : ""}>
                    <td className="checkbox-cell">
                      <input
                        ...
                        onChange={(e) => onRiskSelectionChange(risk.id, e.target.checked)}
                      />
                    </td>
                    <td className="description-cell">
                      <textarea
                        className="risk-description-input"
                        value={currentRisk.description}
                        onChange={(e) => handleFieldChange(risk.id, 'description', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'description', e.target.value)}
                    ...
                    <td className="category-cell">
                      <input
                        ...
                        onChange={(e) => handleFieldChange(risk.id, 'category', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'category', e.target.value)}
                    ...
                    <td className="treatment-cell">
                      <textarea
                        className="treatment-strategy-input"
                        value={currentRisk.treatmentStrategy}
                        onChange={(e) => handleFieldChange(risk.id, 'treatmentStrategy', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'treatmentStrategy', e.target.value)}
                    ...
                    <td className="asset-value-cell">
                      <input
                        ...
                        value={currentRisk.assetValue || ''}
                        onChange={(e) => handleFieldChange(risk.id, 'assetValue', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'assetValue', e.target.value)}
                        placeholder="Enter values in USD"
                      />
                    </td>
                    <td className="department-cell">
                      <input
                        ...
                        value={currentRisk.department || ''}
                        onChange={(e) => handleFieldChange(risk.id, 'department', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'department', e.target.value)}
                    ...
                    <td className="risk-owner-cell">
                      <input
                        type="text"
                        className="risk-owner-input"
                        value={currentRisk.riskOwner || ''}
                        onChange={(e) => handleFieldChange(risk.id, 'riskOwner', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'riskOwner', e.target.value)}
                    ...
                    <td className="target-date-cell">
                      <input
                        ...
                        value={currentRisk.targetDate || ''}
                        onChange={(e) => handleFieldChange(risk.id, 'targetDate', e.target.value)}
                        onBlur={(e) => handleFieldBlur(risk.id, 'targetDate', e.target.value)}
                      />
                    </td>
        ...
        <div className="risk-table-footer">
          <button 
            ...
            onClick={() => {
              ...
              const selectedEditedRisks = risks.map(risk => {
