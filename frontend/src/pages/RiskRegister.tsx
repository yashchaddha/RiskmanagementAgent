...
export const RiskRegister: React.FC<RiskRegisterProps> = ({ onClose }) => {
  ...
  const fetchFinalizedRisks = async () => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      const response = await fetch('/api/risks/finalized', {
        headers: {
  ...
  const filteredRisks = finalizedRisks.filter(risk => {
    const matchesSearch = risk.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         (risk.department && risk.department.toLowerCase().includes(searchTerm.toLowerCase())) ||
                         (risk.riskOwner && risk.riskOwner.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="risk-register-content">
        ...
        {error && (
          <div className="error-message">
            <p>❌ {error}</p>
            <button onClick={fetchFinalizedRisks} className="retry-btn">Retry</button>
          </div>
        )}
                  ...
                  {filteredRisks.map((risk) => (
                    <div key={risk.id} className="risk-card">
                      <div className="risk-card-header">
                        <span className={`category-badge ${getCategoryClass(risk.category)}`}>
                          {risk.category}
                        </span>
                        <div className="risk-levels">
                          <span className={`risk-level ${getRiskLevelClass(risk.likelihood)}`}>
                            L: {risk.likelihood}
                          </span>
                          <span className={`risk-level ${getRiskLevelClass(risk.impact)}`}>
                            I: {risk.impact}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="risk-card-details">
                        ...
                        <div className="detail-row">
                          ...
                          <span className={`progress-badge progress-${risk.riskProgress?.toLowerCase().replace(' ', '-')}`}>
                            {risk.riskProgress || 'Identified'}
                          </span>
                        </div>
                        <div className="detail-row">
                          <span className="detail-label">Residual Exposure:</span>
                          <span className={`exposure-badge exposure-${risk.residualExposure?.toLowerCase().replace(' ', '-')}`}>
                            {risk.residualExposure || 'Not specified'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="risk-card-footer">
                        <span className="risk-date">
                          Finalized: {formatDate(risk.createdAt)}
                        </span>
                      </div>
                    </div>
                  ))}
